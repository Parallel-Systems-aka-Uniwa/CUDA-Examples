...
__global__ void share_ary_oper(int *ary, int *ary_out)
{
    // Thread index
        int tx = threadIdx.x;
...
int main(){
        ...
        cudaMalloc((void **)&device_array, size); cudaMalloc((void **)&device_array_out, size);
        ...
        cudaFree(device_array); cudaFree(host_array); cudaFree(device_array_out);

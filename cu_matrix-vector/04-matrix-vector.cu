...
int main(int argc, char *argv[])
{
	 ...
	 *  - Allocate memory for a matrix on the device.
	...
	err = cudaMalloc(&matrix_d, N * N * sizeof(int));
	if (err != cudaSuccess) {
	...
	err = cudaMemcpy(matrix_d, matrix_h, N * N * sizeof(int), cudaMemcpyHostToDevice);
        if (err != cudaSuccess) {
		printf("Could not copy input vector to device.\n");
		exit(1);
        }
